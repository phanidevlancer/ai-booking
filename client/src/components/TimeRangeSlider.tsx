...
export default function TimeRangeSlider({ timeRange, onTimeRangeChange }: TimeRangeSliderProps) {
  ...
  useEffect(() => {
    if (sliderRef.current) {
      const percentage = (timeRange / 24) * 100;
    ...
    <div>
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-4">
          <span className="text-sm text-muted-foreground w-12">00:00</span>
          <input
            ref={sliderRef}
            type="range"
            min="0"
          ...
          />
          <span className="text-sm text-muted-foreground w-12">24:00</span>
        </div>
